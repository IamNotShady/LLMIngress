  ...
  return (
    ...
    >
      <form action="/api/virtual-models" method="post">
        ...
        {editing && policy ? <input type="hidden" name="routePolicyId" value={policy.id} /> : null}
        ...
        <div className="mt-[14px] grid grid-cols-2 gap-4">
          ...
          >
            <SelectInput name="endpointProtocol" defaultValue={protocol}>
              {PROTOCOLS.map((value) => (
          ...
          <Field label="STRATEGY" hint={strategyRouteNote[policy?.strategy ?? "load_balance"]}>
            <SelectInput name="strategy" defaultValue={policy?.strategy ?? "load_balance"}>
        ...
        <div className="mt-2 rounded-xs border border-rule bg-track px-3 pb-2 pt-1">
