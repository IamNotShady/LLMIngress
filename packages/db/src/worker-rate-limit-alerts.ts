...
export async function evaluateRateLimitAlerts(
  ...
  for (const candidate of candidates) {
    ...
    if (await rateLimitAlertAlreadyQueued(options.databaseUrl, event.payload.alertKey)) {
    ...
    const result = await client.query<{ exists: boolean }>(
      `
        select exists (
          ...
          where event_type = 'rate_limit_high_frequency'
...
function readPositiveIntegerPayload(value: unknown, name: string): number {
  if (typeof value !== "number" || !Number.isInteger(value) || value <= 0) {
    throw new JobHandlerError(
      "rate_limit_alerts_invalid_payload",
      `Rate limit alert payload ${name} must be a positive integer.`,
    );
  }
  return value;
}
