...
- Provider health is connection-scoped for API keys, OAuth tokens, and Local logical connections; model and Provider-level health were removed. Worker probes up to three ranked models, stores only unhealthy summaries, retries at 5/10/30/60 minutes, removes state on recovery, and persists stale or disabled jobs as canceled. Gateway filters confirmed unhealthy connections and probes credential failures; Console aggregates connection status per Provider.
...
database/real-process E2E checks), `pnpm run verify` (63 files/337 tests), and all 9 milestone
