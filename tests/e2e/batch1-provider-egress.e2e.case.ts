...
const apiKey = "llmi_v1_batch1_provider_egress_key_001";

// Batch 1 Feature A egress: glm_coding + qwen_token_plan are OpenAI
// chat_completions api_key providers. Both must route egress to
...
const scenarios: EgressScenario[] = [
  {
    baseUrlSuffix: "/glm/coding/paas/v4",
    displayName: "GLM Coding Plan",
    expectedProviderPath: "/glm/coding/paas/v4/chat/completions",
    id: "glm_coding",
    modelDisplayName: "GLM 4.6",
    modelId: "glm-4.6",
    providerApiKey: "sk-glm-coding-egress-001",
    providerKey: "glm_coding",
    providerTemplateId: "glm_coding",
    requestId: "req_batch1_glm_coding_001",
    virtualModelDisplayName: "Batch1 GLM Coding",
    virtualModelName: "batch1-glm-coding",
  },
  {
    baseUrlSuffix: "/qwen/token-plan/compatible-mode/v1",
    displayName: "Qwen Token Plan",
    expectedProviderPath: "/qwen/token-plan/compatible-mode/v1/chat/completions",
    id: "qwen_token_plan",
    modelDisplayName: "Qwen3 Coder Plus",
    modelId: "qwen3-coder-plus",
    providerApiKey: "sk-sp-qwen-token-plan-egress-001",
    providerKey: "qwen_token_plan",
    providerTemplateId: "qwen_token_plan",
    requestId: "req_batch1_qwen_token_plan_001",
    virtualModelDisplayName: "Batch1 Qwen Token Plan",
    virtualModelName: "batch1-qwen-token-plan",
  },
];
...
test("batch 1 glm_coding and qwen_token_plan route chat_completions egress to a mocked upstream", async () => {
  const fixture = await createTestPostgresFixture({
    databaseNamePrefix: `llmingress_batch1_egress_${randomUUID().replaceAll("-", "_")}`,
  });
...
async function seedRoutes(fixture: Fixture, fakeProviderUrl: string): Promise<void> {
  ...
  await fixture.query(
    "insert into api_keys (id, name, key_prefix, key_hash, enabled) values ($1, 'Batch1 Egress ApiKey', left(gen_random_uuid()::text, 12), gen_random_uuid()::text, true)",
    [seedApiKeyId],
  );
  ...
  await fixture.query(
    "insert into config_versions (version, source, description) values (1, 'console', 'Batch1 egress config')",
  );
}
