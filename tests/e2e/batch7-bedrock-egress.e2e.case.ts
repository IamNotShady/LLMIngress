...
const apiKey = "llmi_v1_batch7_bedrock_egress_key_001";

// Batch 7 egress: AWS Bedrock routes the OpenAI chat_completions face to
// base + /chat/completions with a Bearer credential against a mocked upstream,
// and records a succeeded request_activity row.
type EgressScenario = {
...
const scenarios: EgressScenario[] = [
  {
    baseUrlSuffix: "/bedrock/v1",
    displayName: "AWS Bedrock",
    expectedProviderPath: "/bedrock/v1/chat/completions",
    id: "bedrock",
    modelDisplayName: "Claude Opus 4.8",
    modelId: "global.anthropic.claude-opus-4-8",
    providerApiKey: "ABSKBEDROCKEGRESS000000000000001",
    providerKey: "bedrock",
    providerTemplateId: "bedrock",
    requestId: "req_batch7_bedrock_001",
    virtualModelDisplayName: "Batch7 Bedrock",
    virtualModelName: "batch7-bedrock",
  },
];
...
test("batch 7 bedrock routes chat_completions egress to a mocked upstream", async () => {
  const fixture = await createTestPostgresFixture({
    databaseNamePrefix: `llmingress_batch7_egress_${randomUUID().replaceAll("-", "_")}`,
  });
...
async function seedRoutes(fixture: Fixture, fakeProviderUrl: string): Promise<void> {
  ...
  await fixture.query(
    "insert into api_keys (id, name, key_prefix, key_hash, enabled) values ($1, 'Batch7 Egress ApiKey', left(gen_random_uuid()::text, 12), gen_random_uuid()::text, true)",
    [seedApiKeyId],
  );
  ...
  await fixture.query(
    "insert into config_versions (version, source, description) values (1, 'console', 'Batch7 egress config')",
  );
}
