    ...
    await withProcessLock("llmingress-console-next-dev", async () => {
      ...
      try {
        ...
        try {
          ...
          await expect(
            page.getByRole("heading", { name: "req_console_success_046" }),
          ).toBeVisible();
          await expect(page.getByText("Provider: Console OpenAI")).toBeVisible();
          await expect(page.getByText("Model hit: GPT 4.1 Nano (gpt-4.1-nano)")).toBeVisible();
          await expect(page.getByText("105 total tokens (5 input, 100 output)")).toBeVisible();
          await expect(page.getByText("Cost: $0.00004050")).toBeVisible();
          await expect(
            page.getByText("cost_first route selected cheapest eligible candidate 2."),
          ).toBeVisible();
          await expect(
            page.getByText(
          ...
          await expect(page.getByText("Error code: None")).toBeVisible();

          await page.getByRole("link", { name: "req_console_failure_046" }).click();
          await expect(
            page.getByRole("heading", { name: "req_console_failure_046" }),
          ).toBeVisible();
          await expect(page.getByText("Error code: provider_request_failed")).toBeVisible();
        } finally {
          await context.close();
        }
      } finally {
