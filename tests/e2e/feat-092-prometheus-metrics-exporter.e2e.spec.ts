...
test("prometheus metrics exposes gateway worker request latency cost fallback health job metrics", async () => {
  const fixture = await createTestPostgresFixture({
    databaseNamePrefix: `llmingress_prometheus_metrics_${randomUUID().replaceAll("-", "_")}`,
  });
  ...
  try {
    ...
    try {
      ...
      expect(body).not.toContain("sk-secret-provider-092");
      expect(body).not.toContain("llmi_secret092");
      expect(body).not.toContain("https://secret-provider.example/v1");
      expect(body).not.toContain("sensitive upstream failure");
    } finally {
...
async function seedPrometheusMetricsData(fixture: Fixture): Promise<void> {
  ...
  await fixture.query(
    `
      insert into fallback_events (
        ...
        error_message, failed_before_first_byte
      )
  ...
  await fixture.query(
    `
      ...
      values (
        ...
        '{"apiKey":"sk-secret-provider-092"}'::jsonb, '{"ok":true}'::jsonb, now()
      )
    `,
    [ids.jobId],
  );
  ...
  await fixture.query(
    `
      insert into jobs (
        id, job_type, status, trigger, payload, error_code, error_message, completed_at
      )
      values (
  ...
  await fixture.query(
    `
      insert into job_attempts (
        id, job_id, attempt_number, worker_id, status, error_code, error_message,
