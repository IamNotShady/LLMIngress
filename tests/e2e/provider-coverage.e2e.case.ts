...
import {
  buildProviderCoverageSmokePlan,
...
test("provider coverage routes core providers local providers and validates long tail templates available", async () => {
  const fixture = await createTestPostgresFixture({
    databaseNamePrefix: `llmingress_provider_smoke_${randomUUID().replaceAll("-", "_")}`,
  });
  const fakeProvider = await createFakeProviderServer();
  const plan = buildProviderCoverageSmokePlan(fakeProvider.url);

  try {
    await runMigrations({ databaseUrl: fixture.databaseUrl });
    await seedProviderCoverageRoutes(fixture, plan.providerScenarios);
...
async function seedProviderCoverageRoutes(
  fixture: Fixture,
  ...
  await fixture.query(
    "insert into config_versions (version, source, description) values (1, 'console', 'Provider coverage smoke config')",
  );
}

async function requestScenario(
  baseUrl: string,
  scenario: ProviderCoverageScenario,
): Promise<Response> {
  const endpoint =
