...
describe("gateway cohesion fitness", () => {
  it("loads gateway source files for structural assertions", () => {
    expect(src("packages/gateway-runtime/src/gateway-chat-completions.ts")).toContain(
      "executeGatewayOpenAIChatCompletion",
    );
  });

  it("credential subsystem lives outside the chat endpoint module", () => {
    const chat = src("packages/gateway-runtime/src/gateway-chat-completions.ts");
    ...
    for (const file of ["gateway-embeddings", "gateway-responses", "gateway-messages"]) {
      expect(src(`packages/gateway-runtime/src/${file}.ts`)).not.toContain(
        'from "./gateway-chat-completions',
      );
    }
    const streamingChatImport = src("packages/gateway-runtime/src/gateway-streaming.ts").match(
  ...
  it("uses one gateway error code union without casts", () => {
    for (const file of [
      ...
      const content = src(`packages/gateway-runtime/src/${file}.ts`);
  ...
  it("keeps SSE parsing, baseline selection, and budget math out of the usage recorder", () => {
    const recorder = src("packages/gateway-runtime/src/gateway-usage-recorder.ts");
  ...
  it("reads enabled gateway agent limits once for rate-limit and budget execution", () => {
    expect(src("packages/gateway-runtime/src/gateway-agent-limits.ts")).toMatch(
      /from agent_limits/,
    );
    expect(src("packages/gateway-runtime/src/gateway-agent-limits.ts")).toContain(
      "enforceGatewayAgentLimits",
    );
    expect(existsSync("packages/gateway-runtime/src/gateway-rate-limits.ts")).toBe(false);
    expect(existsSync("packages/gateway-runtime/src/gateway-budgets.ts")).toBe(false);
    expect(src("packages/gateway-runtime/package.json")).not.toContain("./gateway-budgets");
  ...
  it("resolves provider dialects through the registry, not string dispatch", () => {
    const streaming = src("packages/gateway-runtime/src/gateway-streaming.ts");
  ...
  it("owns stream wrapper composition in one module", () => {
    ...
    expect(src("packages/gateway-runtime/src/gateway-stream-pipeline.ts")).not.toContain(
  ...
  it("centralizes gateway env readers and agent naming", () => {
    for (const file of [
      ...
      expect(src(`packages/gateway-runtime/src/${file}.ts`)).not.toMatch(
        /process\.env\.(GATEWAY|LLMINGRESS)_/,
      );
    }
