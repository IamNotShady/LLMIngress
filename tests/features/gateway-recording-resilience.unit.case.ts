...
const recordingMocks = vi.hoisted(() => ({ enabled: false }));

vi.mock("@llmingress/db/client", async (importOriginal) => {
  const actual = await importOriginal<typeof import("@llmingress/db/client")>();
  return {
    ...actual,
    getPostgresPool: vi.fn((...args: Parameters<typeof actual.getPostgresPool>) =>
      recordingMocks.enabled ? { query: postgresQueryMock } : actual.getPostgresPool(...args),
    ),
  };
});
...
vi.mock("@llmingress/gateway-runtime/gateway-api-key-limits", async (importOriginal) => {
  ...
  return {
    ...
    recordGatewayBudgetUsage: vi.fn(
      (...args: Parameters<typeof actual.recordGatewayBudgetUsage>) =>
        recordingMocks.enabled
          ? recordGatewayBudgetUsageMock(...args)
          : actual.recordGatewayBudgetUsage(...args),
    ),
  };
});
